...
// 不同监控类型的默认阈值配置
const DEFAULT_THRESHOLDS: Record<MonitorType, { low: number; high: number; confidence: number }> = {
  temperature: { low: 0, high: 50, confidence: 0.3 },
  humidity: { low: 20, high: 80, confidence: 0.3 },
  pressure: { low: 0, high: 10, confidence: 0.3 },
  vibration: { low: 0, high: 5, confidence: 0.3 },
  flow: { low: 0, high: 100, confidence: 0.3 }
}
...
export function statisticalTuning(
  ...
  if (values.length < 10) {
    return DEFAULT_THRESHOLDS[monitorType]
  }
